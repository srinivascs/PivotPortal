...
public class ReportsPostalFrieghtApplication extends Page{


	public ReportsPostalFrieghtApplication(WebDriver driver, Log log) throws InvalidFormatException, IOException {
 			super(driver, log);
 	} 
 	@Override
 	protected void load() {}
	...
	public boolean ReportGroupByApplication(String AccNo,String Testname) throws Exception {
	    ...
	    if (doesElementExist2(properties.getProperty("ReportTypes"), 5)) {	    
			...
			if (doesElementExist2(properties.getProperty("ReportTypeoptions"), 5)) {
				...
				for (WebElement lnk:selopts) {
					if (lnk.getText().contains("PostageFreightApplication")) {
						((JavascriptExecutor) driver).executeScript("arguments[0].click()", lnk);
						PleasewaitDisappear();
	    ...
	    if ((Initialization.Browser.equals("IE")) || (Initialization.Browser.equals("ie")) || (Initialization.Browser.equals("InternetExplorer")) || (Initialization.Browser.equals("internetexplorer")) || (Initialization.Browser.equals("INTERNETEXPLORER"))) {
	    	...
	    	if (doesElementExist(properties.getProperty("OrderNumIE"), 5)) {
				...
				log.logLine(Testname, true, "Clicking on the Options dropdown list in query parameters field is failed");
				driver.switchTo().defaultContent();
				throw new Exception("Clicking on the Options dropdown list in query parameters field is failed");
				}
	    ...
	    if (doesElementExist2(properties.getProperty("ChkPstgAppln"), 5)) {
    		List<WebElement> Pstgapp = driver.findElements(By.cssSelector(properties.getProperty("ChkPstgAppln")));
    		 for (WebElement lnk:Pstgapp) {
    			 if (lnk.getText().equals("Postage by Application")){
    				 log.logLine(Testname, false, "Reading the Data as "+lnk.getText()+" Under Reports Column");
    				 break;
    			 }
    		 }
	    ...
	    if (doesElementExist2(properties.getProperty("ChkPstgAppln"), 5)) {
	  	    WebElement Pstgapp = driver.findElement(By.cssSelector(properties.getProperty("ChkPstgAppln")));
	    ...
	    if (doesElementExist2(properties.getProperty("RunButton"), 5)) {	  
		    ...
		    if (browserexist) {
			    ...
			    if (winHandle!=firstWinHandle){
					...
					try {			
				        File fileTemp1 = new File(System.getProperty("user.home")+"/Downloads/PostageFreightApplication.csv");
				        if (fileTemp1.exists()){
				        	fileTemp1.delete();
				        	log.logLine(Testname, false, "The PostageFreightApplication.csv exists and has been deleted");
				        	}else{
						    	log.logLine(Testname, false, " PostageFreightApplication.csv file does not exists");
					    	}
			        
			        File fileTemp2 = new File(System.getProperty("user.home")+"/Downloads/PostageFreightApplication.pdf");
			        if (fileTemp2.exists()){
			        	fileTemp2.delete();
			        	log.logLine(Testname, false, "The PostageFreightApplication.pdf exists and has been deleted");
		        	}else{
				    	log.logLine(Testname, false, " PostageFreightApplication.pdf file does not exists");
			    	}
			        
			        File fileTemp3 = new File(System.getProperty("user.home")+"/Downloads/PostageFreightApplication.xls");
			        if (fileTemp3.exists()){
			        	fileTemp3.delete();
			        	log.logLine(Testname, false, "The PostageFreightApplication.xls exists and has been deleted");
		        	}else{
				    	log.logLine(Testname, false, " PostageFreightApplication.xls file does not exists");
			    	}
			       } catch(Exception e){
			        // if any error occurs
			        e.printStackTrace();
		  ...
		  */  
			...
			Thread.sleep(8000);
				try {			
			        File fileTemp1 = new File(System.getProperty("user.home")+"/Downloads/PostageFreightApplication.csv");
			        if (fileTemp1.exists()){
			        	log.logLine(Testname, false, "The Saved PostageFreightApplication.csv file exists ");
			        	}else{
					    	log.logLine(Testname, true, "PostageFreightApplication.csv file does not exists");
				    	}
		        
		        File fileTemp2 = new File(System.getProperty("user.home")+"/Downloads/PostageFreightApplication.pdf");
		        if (fileTemp2.exists()){
		           	log.logLine(Testname, false, "The Saved PostageFreightApplication.pdf file exists");
	        	}else{
			    	log.logLine(Testname, true, "PostageFreightApplication.pdf file does not exists");
		    	}
		        
		        File fileTemp3 = new File(System.getProperty("user.home")+"/Downloads/PostageFreightApplication.xls");
		        if (fileTemp3.exists()){
		       
		        	log.logLine(Testname, false, "The Saved PostageFreightApplication.xls file exists");
	        	}else{
			    	log.logLine(Testname, true, "PostageFreightApplication.xls file does not exists");
		    	}
		       } catch(Exception e){
		        // if any error occurs
		        e.printStackTrace();
